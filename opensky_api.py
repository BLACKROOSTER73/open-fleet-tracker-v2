...
# need `git` at runtime to install it via a `git+https://...` pip URL (which
# is fragile inside minimal/unprivileged containers). It is kept in the same
# directory as fleet_tracker.py so `from opensky_api import ...` resolves to
# this file with zero code changes elsewhere in the project.
#
# Upstream source: https://github.com/openskynetwork/opensky-api
# To update: replace this file with the latest
# python/opensky_api.py from that repo.
# ---------------------------------------------------------------------------
import calendar
import json
import logging
